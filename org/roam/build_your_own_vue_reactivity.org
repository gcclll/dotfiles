...
// 是不是应该收集信赖，比如正在执行的 effect 就不该重复收集
...
每个订阅函数最终都会被封装成一个 ~ReactiveEffect~ 被收集到 ~targetMap -> depsMap~ 中
...
class ReactiveEffect {
  ...
  run() {    (ref:effect-run)
    ...
    console.log(`effect|run| active=${this.active}, shouldTrack=${shouldTrack}`)
    if (!this.active) {
      return this.fn()
    }
...
function createGetter(isReadonly = false, shallow = false) {
  ...
  return function get(target, key, receiver) {
    ...
    console.log(`createGetter| get, key=${key}, value=${res}`)
...
接下来是如何结合 track 各 trigger 实现信赖收集和当值发生变化时去触发某种行为(DOM
更新)。
...
[[trigger][trigger]]: set 设值操作时会去触发 track 收集到的信赖并执行它们，这个执行的动作实际
...
[[effect][effect]]: 将一个函数封装成 [[ReactiveEffect][ReactiveEffect]] 类，调用的时候会立即执行一个 fn 收集信赖。
...
function track(target, type, key) {
  if (shouldTrack && activeEffect) {
    ...
    console.log(`track| type=${type}, key=${key}, targetMap=${targetMap}, dep=${dep}`)
...
function trackEffects(dep, debuggerEventExtraInfo) {
  ...
  console.log(`trackEffects| activeEffect=${activeEffect}, dep=${dep.size}, activeEffect.deps=${activeEffect.deps.length}`)
}
#+end_src
** trigger()
...
function trigger(target, type, key, newValue, oldValue, oldTarget) {
  ...
  if (!depsMap) {
    // 如果是空的表明这个对象从没有被取过值也就没有收集到任何信赖
    return
  }

  console.log(`trigger| type=${type}, key=${key}, newValue=${newValue}, oldValue=${oldValue}`)
    ...
    switch (type) {
      ...
      case 'delete':
        if (!isArray(target)) {
          deps.push(depsMap.get(ITERATE_KEY))
          if (isMap(target)) {
            deps.push(depsMap.get(MAP_KEY_TERATE_KEY))
          }
        }
        break
      case 'set':
  ...
  console.log(`trigger|deps| deps=${deps}`)
  if (deps.length === 1) {
    if (deps[0]) {
...
function triggerEffects(dep, debuggerEventExtraInfo) { (ref:triggerEffects)
  for (const effect of Array.isArray(dep) ? dep : [...dep]) {
    if (effect !== activeEffect || effect.allowRecurse) { (ref:triggerEffects-allowRecurse)
      console.log(`triggerEffects| allowRecurse=${effect.allowRecurse}, dep=${dep}, effect=${effect}`)
...
| code                | testing     |
|---------------------+-------------|
| [[collection-getP2][collection get]]      | [[test-collection-getP2][go test 🛬]] |
| [[collection-hasP2][collection has]]      | [[test-collection-hasP2][go test 🛬]] |
| [[collection-sizeP2][collection size]]     | [[test-collection-sizeP2][go test 🛬]] |
| [[collection-addP2][collection add]]      | [[test-collection-addP2][go test 🛬]] |
| [[collection-setP2][collection set]]      | [[test-collection-setP2][go test 🛬]] |
| [[collection-deleteP2][collection delete]]   | [[test-collection-deleteP2][go test 🛬]] |
| [[collection-clearP2][collection clear]]    | [[test-collection-clearP2][go test 🛬]] |
| [[collection-forEachP2][collection forEach]]  | [[test-collection-forEachP2][go test 🛬]] |
| [[collection-iteratorP2][collection iterator]] | [[test-collection-iteratorP2][go test 🛬]] |
...
4. 下面 createStter 是在 fn 中调用了 observed.count++ -> 等价于 ~observed.count =
   ...
   effects 信赖，但是在 fn 中调用 effect(fn) 的话会导致重新收集了 fn 进 reactive
   变量的信赖表。所以在 ~trigger->triggerEffects~ 中增加了一个选项来控制是不是要放
   ...
   #+begin_src js -n -r :exports both
    ...
    const fn = () => {
      ...
      console.log(`run times ${++times}, count=${observed.count} ...`)

      if (++observed.count < 3) {
        console.log(`allowRecurse option opened...`)
        // 允许
        effect(fn)
        return
      }
...
#+begin_details set
#+name: test-collection-setP2
#+begin_src js -n -r
console.log('xx')
#+end_src
#+end_details

#+begin_details delete
#+name: test-collection-deleteP2
#+begin_src js -n -r
console.log('xx')
#+end_src
#+end_details
...
#+begin_details clear
#+name: test-collection-clearP2
#+begin_src js -n -r
console.log('xx')
#+end_src
#+end_details

#+begin_details forEach
#+name: test-collection-forEachP2
#+begin_src js -n -r
console.log('xx')
#+end_src
#+end_details

#+begin_details iterator
#+name: test-collection-iteratorP2
#+begin_src js -n -r
console.log('xx')
#+end_src
#+end_details
